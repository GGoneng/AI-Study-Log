...
class VAEModel(nn.Module):
    ...
    def forward(self, inputs):
        ...
        _, encode = self.encoder(inputs)
        
        if self.encoder.bidirectional:
            encode = torch.cat([encode[-2], encode[-1]], dim=-1)
        else:
            encode = encode[-1]
        ...
        decode, _ = self.decoder(reparameter)

        reconstruction = self.output(decode.squeeze(1))
...
def VAE_loss(reconstruction, target, mu, logvar):
    ...
    loss_val = MSE_loss(reconstruction, target)
